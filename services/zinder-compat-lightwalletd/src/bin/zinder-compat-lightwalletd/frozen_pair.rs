//! Frozen secondary-pair lifecycle for production lightwalletd serving.
//!
...
#[derive(Clone, Debug)]
pub(crate) struct FrozenPairConfig {
...
#[derive(Debug, Error)]
pub(crate) enum FrozenPairError {
    ...
    #[error("failed to create frozen secondary generation directory {path}: {source}")]
    ...
    #[error("frozen secondary pair slot is unavailable")]
    PairSlotUnavailable,
...
pub(crate) enum PairConvergence {
...
impl FrozenPairManager {
    ...
    pub(crate) async fn bootstrap(
    ...
    ) -> Result<(Self, FrozenPairSlot), FrozenPairError> {
        let writer_status =
        ...
        manager.prepare_candidate(0).await?;
        manager.converge_and_publish(0).await?;
        let Some(read_pairs) = manager.read_pairs.clone() else {
            return Err(FrozenPairError::PairSlotUnavailable);
        };
        Ok((manager, read_pairs))
    }
    ...
    #[must_use = "await the manager on shutdown so secondary handles close before process exit"]
                    ...
                    () = tokio::time::sleep(self.config.catchup_interval) => {
                        if let Err(error) = self.refresh_once().await {
                            ...
                            tracing::warn!(
                                ...
                                event = "frozen_pair_refresh_failed",
                                error = %error,
                                "inactive frozen pair refresh failed; retaining the prior immutable pair"
                            );
                        }
                    }
                }
    ...
    async fn refresh_once(&mut self) -> Result<(), FrozenPairError> {
        let read_pairs = self
            .read_pairs
            .as_ref()
            .ok_or(FrozenPairError::PairSlotUnavailable)?;
        let active_pair = read_pairs.load_full();
    ...
    async fn prepare_candidate(&mut self, generation: usize) -> Result<bool, FrozenPairError> {
        ...
        let candidate = tokio::task::spawn_blocking(move || {
            std::fs::create_dir_all(&canonical_generation_path).map_err(|source| {
                FrozenPairError::SecondaryGenerationDirectoryCreate {
            ...
            Ok::<_, FrozenPairError>(Box::new(SecondaryPairCandidate { canonical, wallet }))
        })
        .await??;
    ...
    async fn converge_and_publish(&mut self, generation: usize) -> Result<(), FrozenPairError> {
        let deadline = Instant::now() + self.config.convergence_timeout;
        let mut last_outcome = PairConvergence::ReplicaBehind;
        for attempt in 0..u32::from(self.config.convergence_attempts.get()) {
            ...
            match self.candidate_convergence(generation)? {
                Ok(()) => {
                    ...
                    last_outcome = PairConvergence::ReplicaBehind;
        ...
        Err(FrozenPairError::ConvergenceTimedOut { last_outcome })
    }

    async fn catch_up_candidate(&mut self, generation: usize) -> Result<(), FrozenPairError> {
        ...
        let candidate_task_outcome = tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, FrozenPairError>(candidate)
        })
        .await;
        match candidate_task_outcome {
            Ok(Ok(candidate)) => {
                metrics::histogram!(
                    "zinder_compat_lightwalletd_frozen_pair_catchup_duration_seconds",
                    "status" => "ok"
                )
            ...
            Err(error) => Err(FrozenPairError::CandidateTask(error)),
        }
    }
    ...
    fn candidate_convergence(
        &self,
        generation: usize,
    ) -> Result<Result<(), PairConvergence>, FrozenPairError> {
    ...
    fn candidate_wallet_source(
        &self,
        generation: usize,
    ) -> Result<WalletCanonicalSourceIdentity, FrozenPairError> {
        let SecondaryGenerationState::Candidate { candidate } = &self.generations[generation].state
        else {
            return Err(FrozenPairError::CandidateUnavailable { generation });
        };
    ...
    fn publish_candidate(&mut self, generation: usize) -> Result<(), FrozenPairError> {
        let state = std::mem::replace(
            &mut self.generations[generation].state,
            SecondaryGenerationState::Vacant,
        );
        ...
        metrics::counter!("zinder_compat_lightwalletd_frozen_pair_publications_total").increment(1);
        tracing::info!(
            target: "zinder::compat_lightwalletd",
            event = "frozen_pair_published",
            generation,
    ...
    fn update_active_readiness(
        ...
        let Some(writer_fence) = writer_status.fence.as_ref() else {
            return Err(FrozenPairError::WriterStatusInvalid);
        };
        ...
        if writer_fence.chain_epoch_id < active_epoch {
            return Err(FrozenPairError::WriterFenceMismatch);
        }
    ...
    fn record_refresh_failure(&self, error: &FrozenPairError) {
        let visible_height = self.read_pairs.as_ref().map(|read_pairs| {
            read_pairs
                .load_full()
                .canonical_fence()
                .visible_tip()
        ...
        metrics::counter!(
            "zinder_compat_lightwalletd_frozen_pair_refresh_total",
            "status" => "error",
            "error_class" => frozen_pair_error_class(error)
        )
        .increment(1);
    }
}
...
impl SecondaryGeneration {
    fn new(config: &FrozenPairConfig, generation: usize) -> Self {
...
fn publish_immutable_pair<T: Send + Sync + 'static>(
...
impl CanonicalWriterStatusClient {
    ...
    async fn fetch(&mut self) -> Result<CanonicalWriterStatusResponse, FrozenPairError> {
        let started_at = Instant::now();
        let outcome = self
            .client
            ...
            .map_err(FrozenPairError::WriterStatusRpc);
        metrics::histogram!(
    ...
    match error {
        FactFirstPairAdmissionError::WalletSourceMismatch { canonical, wallet } => {
            ...
            match canonical_event_sequence.cmp(&wallet_event_sequence) {
                std::cmp::Ordering::Less => PairConvergence::ReplicaBehind,
                std::cmp::Ordering::Greater => PairConvergence::ProjectionBehind,
...
fn record_pair_convergence(outcome: PairConvergence) {
    metrics::counter!(
...
fn record_replica_lag(lag_chain_epochs: u64) {
    ...
    metrics::gauge!("zinder_compat_lightwalletd_frozen_pair_replica_lag_chain_epochs").set(lag);
}

/// Returns the fail-closed readiness cause for a refresh error. `None` is
/// reserved for a normally typed replica-behind outcome.
fn refresh_failure_not_ready_cause(error: &FrozenPairError) -> Option<ReadinessCause> {
    match error {
        FrozenPairError::WriterStatusConnect(_) | FrozenPairError::WriterStatusRpc(_) => {
            Some(ReadinessCause::WriterStatusUnavailable)
        }
        FrozenPairError::WriterStatusInvalid
        | FrozenPairError::WriterFenceMismatch
        | FrozenPairError::ConvergenceTimedOut {
            last_outcome: PairConvergence::SchemaOrFenceMismatch,
        } => Some(ReadinessCause::SchemaMismatch),
        FrozenPairError::ConvergenceTimedOut {
            last_outcome: PairConvergence::ReplicaBehind,
        } => None,
        FrozenPairError::ConvergenceTimedOut {
            last_outcome: PairConvergence::ProjectionBehind,
        }
        | FrozenPairError::Canonical(_)
        | FrozenPairError::Wallet(_)
        | FrozenPairError::SecondaryGenerationDirectoryCreate { .. }
        | FrozenPairError::CandidateTask(_)
        | FrozenPairError::PairPublication(_)
        | FrozenPairError::PairSlotUnavailable
        | FrozenPairError::CandidateUnavailable { .. } => Some(ReadinessCause::StorageUnavailable),
    }
}

fn frozen_pair_error_class(error: &FrozenPairError) -> &'static str {
    match error {
        FrozenPairError::Canonical(_) => "canonical_secondary",
        FrozenPairError::Wallet(_) => "wallet_secondary",
        FrozenPairError::SecondaryGenerationDirectoryCreate { .. } => "generation_directory_create",
        FrozenPairError::WriterStatusConnect(_) => "writer_status_connect",
        FrozenPairError::WriterStatusRpc(_) => "writer_status_rpc",
        FrozenPairError::WriterStatusInvalid => "writer_status_invalid",
        FrozenPairError::WriterFenceMismatch => "writer_fence_mismatch",
        FrozenPairError::CandidateTask(_) => "candidate_task",
        FrozenPairError::PairSlotUnavailable => "pair_slot_unavailable",
        FrozenPairError::CandidateUnavailable { .. } => "candidate_unavailable",
        FrozenPairError::ConvergenceTimedOut {
            last_outcome: PairConvergence::ReplicaBehind,
        } => "replica_behind",
        FrozenPairError::ConvergenceTimedOut {
        ...
        FrozenPairError::PairPublication(_) => "pair_publication",
    }
}
...
mod tests {
    ...
    use super::{
        FrozenPairConfig, FrozenPairError, FrozenPairManager, GenerationLease, PairConvergence,
        SecondaryGenerationState, classify_pair_admission, publish_immutable_pair,
        refresh_failure_not_ready_cause, writer_status_matches_source,
    };

    #[test]
    fn exact_source_is_eligible_for_frozen_pair_publication() {
    ...
    #[test]
    fn mutated_writer_fence_is_rejected_before_frozen_pair_publication() {
        let source = source_identity(3, 0x33);
    ...
    fn atomic_publication_keeps_a_retired_generation_until_every_request_arc_drains() {
        ...
        let active_slot = publish_immutable_pair(&mut slot, Arc::clone(&replacement_pair));
    ...
    fn schema_or_fence_convergence_failure_is_not_ready() {
        assert_eq!(
            refresh_failure_not_ready_cause(&FrozenPairError::ConvergenceTimedOut {
                last_outcome: PairConvergence::SchemaOrFenceMismatch,
            }),
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (mut manager, slot) = FrozenPairManager::bootstrap(
            frozen_pair_config(
        ...
        assert_eq!(published_generation_is_reusable(&manager, 0), Some(false));
        assert!(
            canonical_secondary_root
                .join("generation-1")
                .join("canonical")
        ...
        drop(old_pair);
        assert_eq!(published_generation_is_reusable(&manager, 0), Some(true));
        ...
        *digest_byte ^= 0xff;
        ...
        assert!(matches!(failure, FrozenPairError::WriterFenceMismatch));
        manager.record_refresh_failure(&failure);
    ...
    ) -> Result<FrozenPairConfig, Box<dyn std::error::Error>> {
        Ok(FrozenPairConfig {
            canonical_primary_path,
            canonical_secondary_root,
            wallet_primary_path,
            ...
            convergence_attempts: NonZeroU32::new(4)
                ...
                .ok_or("frozen pair convergence attempts must be non-zero")?,
    ...
    fn published_generation_is_reusable(
        manager: &FrozenPairManager,
        generation: usize,
    ) -> Option<bool> {
        match &manager.generations[generation].state {
