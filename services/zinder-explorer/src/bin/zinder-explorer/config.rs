...
/// Resolved storage used by the Explorer materialized-view secondary.
#[derive(Clone, Debug)]
pub(crate) struct ExplorerMaterializedViewStorage {
    /// Canonical storage root containing the ingest-owned materialized-view primary.
    ...
    /// Bounded reader budget for the materialized-view store.
...
pub(crate) enum ExplorerConfigError {
    ...
    #[error("failed to bind explorer gRPC listener at {listen_addr}: {source}")]
    GrpcBind {
    ...
    #[error("explorer gRPC server stopped without a shutdown request")]
    GrpcServerStopped,

    #[error("explorer runtime task `{task}` stopped without a shutdown request")]
    RuntimeTaskStopped { task: &'static str },

    #[error("explorer runtime task `{task}` failed: {source}")]
    RuntimeTaskJoin {
        task: &'static str,
        #[source]
