//! CLI adapter for concrete canonical-fact storage round trips.
...
use zinder_bench::{
    ...
    canonical_fact_round_trip::{
        ...
        rocksdb::{
            ...
            run_rocksdb_canonical_fact_round_trip,
        },
    },
    fixture::FixtureManifest,
    report::{
        BenchmarkReport, CanonicalBlockFactsRoundTripMeasurements,
        CanonicalBlockFactsStorageEvidence, CanonicalFactSequenceDigestSummary, FixtureCachePolicy,
        FixtureSummary, PostgresBenchmarkRuntimeEvidence, RocksDbResourceBudgetSummary,
        StorageCandidateIdentity, build_canonical_block_facts_round_trip_report,
        is_immutable_image_reference, is_valid_benchmark_trial_id,
    },
    rss::peak_rss,
};
...
/// Concrete engine selected for a canonical-fact round trip.
#[derive(Args)]
pub(super) struct CanonicalFactsRoundTripArgs {
    #[command(subcommand)]
    engine: CanonicalFactsRoundTripEngine,
}

#[derive(Subcommand)]
enum CanonicalFactsRoundTripEngine {
    /// Build and validate a fresh sorted-external-SST `RocksDB` candidate.
    #[command(name = "rocksdb")]
    RocksDb(RocksDbCanonicalFactsRoundTripArgs),
    /// Build and validate a fresh binary-COPY `PostgreSQL` candidate.
    Postgres(PostgresCanonicalFactsRoundTripArgs),
}

#[derive(Args)]
struct RocksDbCanonicalFactsRoundTripArgs {
    #[command(flatten)]
    input: CanonicalFactInputArgs,
    /// Fresh candidate store path. Existing paths are rejected.
    #[arg(long)]
    store: PathBuf,
    #[command(flatten)]
    report: CanonicalFactReportArgs,
}

#[derive(Args)]
struct PostgresCanonicalFactsRoundTripArgs {
    #[command(flatten)]
    input: CanonicalFactInputArgs,
...
#[derive(Args)]
struct CanonicalFactReportArgs {
...
pub(super) struct CanonicalFactsRoundTripOutput {
    pub report: BenchmarkReport,
    pub report_path: Option<PathBuf>,
}

pub(super) async fn run_canonical_facts_round_trip(
    args: CanonicalFactsRoundTripArgs,
) -> Result<CanonicalFactsRoundTripOutput, BenchError> {
    let run_started_at_unix_millis = UnixTimestampMillis::now().value();
    match args.engine {
        CanonicalFactsRoundTripEngine::RocksDb(args) => {
            run_rocksdb(args, run_started_at_unix_millis).await
        }
        CanonicalFactsRoundTripEngine::Postgres(args) => {
...
async fn run_rocksdb(
    args: RocksDbCanonicalFactsRoundTripArgs,
    ...
    let round_trip = run_rocksdb_canonical_fact_round_trip(RocksDbCanonicalFactRoundTripConfig {
    ...
    let measurements = CanonicalBlockFactsRoundTripMeasurements {
        ...
        fact_preparation_wall_clock_seconds: round_trip.timings.fact_preparation_seconds,
        fact_persistence_wall_clock_seconds: round_trip.timings.fact_persistence_seconds,
        ...
        logical_fact_bytes: round_trip.logical_fact_bytes,
        physical_storage_bytes: round_trip.physical_storage_bytes,
        persisted_sequence_digest: CanonicalFactSequenceDigestSummary::from_digest(
    ...
    let report = build_canonical_block_facts_round_trip_report(fixture, measurements);
    Ok(CanonicalFactsRoundTripOutput {
        report,
...
async fn run_postgres(
    args: PostgresCanonicalFactsRoundTripArgs,
    run_started_at_unix_millis: u64,
) -> Result<CanonicalFactsRoundTripOutput, BenchError> {
    let block_prepare_concurrency =
        require_nonzero_concurrency(args.input.block_prepare_concurrency)?;
...
fn build_postgres_round_trip_output(
    ...
    round_trip: PostgresCanonicalFactRoundTripResult,
    benchmark_runtime: PostgresBenchmarkRuntimeEvidence,
    report_context: CanonicalFactReportContext,
) -> Result<CanonicalFactsRoundTripOutput, BenchError> {
    let fixture = FixtureSummary::try_from(manifest)?;
    ...
    let measurements = CanonicalBlockFactsRoundTripMeasurements {
        ...
        logical_fact_bytes: round_trip.storage.logical_fact_bytes,
        ...
        benchmark_client_peak_rss: peak_rss(),
        storage_candidate: StorageCandidateIdentity::postgres_canonical_fact_round_trip(),
        software_revision: report_context.software_revision,
        trial_id: report_context.trial_id,
        fixture_cache_policy: report_context.fixture_cache_policy,
        run_started_at_unix_millis: report_context.run_started_at_unix_millis,
...
struct CanonicalFactReportContext {
...
impl CanonicalFactReportArgs {
    fn validate_and_resolve(
        self,
        run_started_at_unix_millis: u64,
    ) -> Result<CanonicalFactReportContext, BenchError> {
...
fn validate_postgres_resource_partition(
    ...
    database_cpu_limit_cores: Option<f64>,
    database_memory_limit_bytes: Option<u64>,
    report: &CanonicalFactReportContext,
) -> Result<(), BenchError> {
    match (
...
fn require_populated_position(
    ...
    let first_height = position.first_height.ok_or_else(|| {
        BenchError::canonical_fact_sequence_mismatch("persisted canonical fact sequence is empty")
    })?;
    let first_hash = position.first_hash.ok_or_else(|| {
        BenchError::canonical_fact_sequence_mismatch(
            "persisted canonical fact sequence has no first hash",
        )
    })?;
    let tip_height = position.tip_height.ok_or_else(|| {
        BenchError::canonical_fact_sequence_mismatch(
            "persisted canonical fact sequence has no tip height",
        )
    })?;
    let tip_hash = position.tip_hash.ok_or_else(|| {
        BenchError::canonical_fact_sequence_mismatch(
            "persisted canonical fact sequence has no tip hash",
        )
    })?;
...
mod tests {
    ...
    use super::{
        CanonicalFactReportContext, CanonicalFactsRoundTripEngine,
    ...
    #[test]
    fn fact_round_trip_engines_have_disjoint_required_arguments() {
        assert!(
            Cli::try_parse_from([
                "zinder-bench",
                "canonical-facts-round-trip",
                "rocksdb",
                "--fixture",
                "fixture",
                "--store",
    ...
    fn postgres_uses_the_credential_safe_environment_default() -> Result<(), clap::Error> {
        ...
        let Command::CanonicalFactsRoundTrip(args) = cli.command else {
            unreachable!("the parsed command must be canonical-facts-round-trip");
        };
        let CanonicalFactsRoundTripEngine::Postgres(args) = args.engine else {
