...
use zinder_bench::canonical_fact_round_trip::rocksdb::{
    RocksDbCanonicalFactRoundTripConfig, run_rocksdb_canonical_fact_round_trip,
...
async fn external_sst_round_trip_publishes_validated_facts_for_a_fresh_reader() -> Result<()> {
    ...
    let candidate_path = candidate_parent.path().join("rocksdb-fact-first");
    ...
    let result = run_rocksdb_canonical_fact_round_trip(RocksDbCanonicalFactRoundTripConfig {
    ...
    let fresh_reader = validate_rocksdb_canonical_fact_round_trip_with_fresh_open(
        &candidate_path,
...
async fn fixture_oracle_mismatch_leaves_the_candidate_unpublished() -> Result<()> {
    ...
    let Err(round_trip_error) =
        run_rocksdb_canonical_fact_round_trip(RocksDbCanonicalFactRoundTripConfig {
            fixture_directory: fixture_directory.path().to_path_buf(),
            candidate_path: candidate_path.clone(),
            block_prepare_concurrency,
            rocksdb_resource_budget,
    ...
    let Err(fresh_reader_error) = validate_rocksdb_canonical_fact_round_trip_with_fresh_open(
        &candidate_path,
        fixture_directory.path(),
        rocksdb_resource_budget,
    ) else {
...
async fn existing_candidate_directory_is_rejected_without_modification() -> Result<()> {
    ...
    let Err(error) = run_rocksdb_canonical_fact_round_trip(RocksDbCanonicalFactRoundTripConfig {
        fixture_directory: fixture_directory.path().to_path_buf(),
