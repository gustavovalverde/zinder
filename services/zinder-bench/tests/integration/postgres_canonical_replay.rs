...
use zinder_bench::canonical_replay_storage::postgres::{
    PostgresCanonicalReplayConfig, run_postgres_canonical_replay_storage,
    validate_postgres_canonical_replay_storage_with_fresh_connection,
};
...
async fn completed_round_trip_supports_a_fresh_reader_and_rejects_schema_reuse() -> Result<()> {
    ...
    let result = run_postgres_canonical_replay_storage(PostgresCanonicalReplayConfig::new(
    ...
    assert!(result.validation.position.logical_replay_bytes > 0);
    assert_eq!(
        result.validation.replay_format_version,
        CanonicalBlockReplayFormatVersion::CURRENT.value()
    );
    assert!(result.storage.replay_table_bytes > 0);
    assert!(result.storage.wal_bytes > 0);
    let fresh_reader = validate_postgres_canonical_replay_storage_with_fresh_connection(
        database_url.as_str(),
        fixture_directory.path(),
    )
    .await?;
    ...
    let Some(reuse_error) =
        run_postgres_canonical_replay_storage(PostgresCanonicalReplayConfig::new(
            fixture_directory.path(),
            database_url.as_str(),
            concurrency,
        ))
