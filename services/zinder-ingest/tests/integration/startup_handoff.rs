...
fn materialized_view_config(
    startup_handoff_lag_blocks: u64,
) -> Result<MaterializedViewReplayConfig> {
...
fn block_summary_head(materialized_view_store: &MaterializedViewStore) -> Result<u32> {
    materialized_view_store
        .last_materialized_height_ascending(BLOCK_SUMMARY_COLUMN_FAMILY)?
        .map(BlockHeight::value)
        .ok_or_else(|| eyre!("materialized-view store materialized no block summary"))
}

fn materialized_view_status(
    materialized_view_store: &MaterializedViewStore,
) -> Result<MaterializedViewStatus> {
    let bytes = materialized_view_store
        .get_materialized_view_status()?
        .ok_or_else(|| eyre!("materialized-view status record missing"))?;
    MaterializedViewStatus::decode(bytes.as_slice())
        .map_err(|error| eyre!("decode materialized-view status: {error}"))
}
...
async fn startup_handoff_returns_with_residual_lag_then_the_tailer_drains_the_rest() -> Result<()> {
    ...
    let materialized_view_store = open_primary_materialized_view_store_for_canonical(
    ...
    catch_up_materialized_view_store_to_canonical_until_handoff(
        ...
        &materialized_view_store,
        materialized_view_config(handoff_lag_blocks)?,
    )
    .await?;

    let residual_head = CANONICAL_HEIGHT - u32::try_from(handoff_lag_blocks)?;
    assert_eq!(
        block_summary_head(&materialized_view_store)?,
        residual_head,
        "startup catch-up must hand off within the handoff lag, not drain the whole debt",
    );

    let status = materialized_view_status(&materialized_view_store)?;
    assert_eq!(
        status.health,
        MaterializedViewHealth::CatchingUp as i32,
...
async fn startup_handoff_returns_immediately_when_already_within_the_handoff_lag() -> Result<()> {
    ...
    catch_up_materialized_view_store_to_canonical_until_handoff(
        store_fixture.chain_store(),
        &materialized_view_store,
        materialized_view_config(u64::from(CANONICAL_HEIGHT))?,
    )
    .await?;
...
async fn ranking_bootstrap_defers_when_replay_trails_the_event_tail() -> Result<()> {
    ...
    let outcome = bootstrap_transparent_address_ranking(
        store_fixture.chain_store(),
        &materialized_view_store,
    )
    .await?;
