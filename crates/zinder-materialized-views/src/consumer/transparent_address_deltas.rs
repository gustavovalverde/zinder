//! `TransparentAddressDeltas` materialized-view consumer.
//!
...
pub const TRANSPARENT_ADDRESS_DELTAS_CONSUMER_NAME: MaterializedViewConsumerName =
    MaterializedViewConsumerName::from_static("transparent_address_deltas");

/// On-disk schema declaration for the transparent-address-deltas consumer.
pub const TRANSPARENT_ADDRESS_DELTAS_SCHEMA: MaterializedViewConsumerSchema =
...
impl TransparentAddressDeltasConsumer {
    ...
    pub fn lifetime_summaries_through(
    ...
    ) -> Result<TransparentAddressDeltasLifetimeBootstrap, MaterializedViewStoreError> {
        ...
        if let Some(address_script_hash) = current_address {
            push_lifetime_summary(&mut summaries, address_script_hash, &accumulator).map_err(
                |reason| MaterializedViewStoreError::ConsumerPayloadDecode {
...
fn lifetime_summary(
    address_script_hash: TransparentAddressScriptHash,
    accumulator: &LifetimeAccumulator,
) -> Result<TransparentAddressLifetimeSummary, MaterializedViewStoreError> {
...
fn source_coverage_through(
    store: &MaterializedViewStore,
    through_height: BlockHeight,
) -> Result<TransparentAddressDeltasSourceCoverage, MaterializedViewStoreError> {
...
mod tests {
    ...
    fn open_store() -> Result<(tempfile::TempDir, MaterializedViewStore)> {
        let tempdir = tempdir()?;
    ...
    fn put_index_height(store: &MaterializedViewStore, height: u32) -> Result<()> {
        store.put_consumer(
            TRANSPARENT_ADDRESS_DELTAS_INDEX_COLUMN_FAMILY,
    ...
    fn lifetime_bootstrap_rejects_malformed_primary_and_index_rows() -> Result<()> {
        ...
        assert!(matches!(
            malformed_primary,
            Err(MaterializedViewStoreError::ConsumerPayloadDecode { name, .. })
