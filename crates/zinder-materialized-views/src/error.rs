...
pub enum MaterializedViewStoreError {
    /// A physical secondary returned a terminal catch-up failure, so its
    /// potentially advanced rows can no longer be served safely.
    #[error("materialized-view secondary reads are fenced after a terminal catch-up failure")]
    SecondaryReadsFenced,
    ...
    /// A persisted checkpoint does not exactly match its retained canonical event.
    #[error(
        "materialized-view chain-event checkpoint does not match retained canonical event at sequence {event_sequence}"
    )]
    ChainEventCheckpointRetainedEventMismatch {
