//! Lightweight wallet tree-state artifact.

use crate::{BlockHash, BlockHeight};

/// One final note-commitment-tree root in source RPC byte order.
...
impl FinalNoteCommitmentRoot {
    ...
    /// Returns the root bytes in their original source RPC byte order.
    #[must_use]
...
/// Commitment tree-state artifact for wallet reads.
