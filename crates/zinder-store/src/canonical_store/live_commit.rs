...
use zinder_core::{
    ...
    CommitmentTreeCheckpoint, NetworkUpgradeActivations, NetworkUpgradeActivationsFingerprintVersion,
    ShieldedProtocol, SubtreeRootArtifact, TransactionLocation, UnixTimestampMillis,
};
...
impl CanonicalEventFence {
    ...
    pub fn decode_persisted(
        encoded: &[u8],
    ) -> Result<Self, CanonicalEventFenceDecodeError> {
        ...
        let sequence_digest_version =
            CanonicalBlockFactsSequenceDigestVersion::try_from(sequence_digest_version_value)
                .map_err(|_| {
                    CanonicalEventFenceDecodeError::UnsupportedSequenceDigestVersion {
