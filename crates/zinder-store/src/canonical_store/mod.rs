...
use zinder_core::{
    ...
    MAX_COMMITMENT_TREE_FRONTIER_FINAL_STATE_BYTES, Network, NetworkUpgradeActivations,
...
impl CanonicalStoreConstructionIdentity {
    ...
    pub fn encode_persisted(self) -> [u8; CANONICAL_STORE_CONSTRUCTION_IDENTITY_RECORD_BYTES] {
        ...
        encoded[7..39].copy_from_slice(&self.network_upgrade_activations_fingerprint.as_bytes());
        encoded[39..41].copy_from_slice(&self.construction_manifest_binding.version.to_be_bytes());
    ...
    pub fn decode_persisted(
        ...
        let network = Network::from_id(network_id)
            .ok_or(CanonicalStoreConstructionIdentityDecodeError::UnknownNetwork { network_id })?;
...
pub enum CanonicalStoreError {
    ...
    SecondaryConstructionIdentityChanged {
        ...
        before: Box<CanonicalStoreConstructionIdentity>,
        /// Structurally valid identity observed after catch-up.
        after: Box<CanonicalStoreConstructionIdentity>,
    },
...
mod tests {
    ...
    fn construction_identity(
        network: Network,
    ) -> Result<CanonicalStoreConstructionIdentity, CanonicalStoreConstructionIdentityDecodeError>
    {
        let mut encoded = [0_u8; CANONICAL_STORE_CONSTRUCTION_IDENTITY_RECORD_BYTES];
        encoded[0] = CANONICAL_STORE_CONSTRUCTION_IDENTITY_RECORD_VERSION;
    ...
    #[test]
    fn construction_identity_persisted_codec_round_trips_and_rejects_unknown_versions()
    -> Result<(), Box<dyn std::error::Error>> {
        let identity = construction_identity(Network::ZcashRegtest)?;
        let encoded = identity.encode_persisted();
        ...
        unsupported[0] = CANONICAL_STORE_CONSTRUCTION_IDENTITY_RECORD_VERSION.saturating_add(1);
        assert!(matches!(
            CanonicalStoreConstructionIdentity::decode_persisted(&unsupported),
            Err(CanonicalStoreConstructionIdentityDecodeError::UnsupportedRecordVersion { .. })
        ));
        Ok(())
    }
