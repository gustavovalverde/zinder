...
use crate::{DeriveStore, DeriveStoreColumnFamily, DeriveStoreError};
...
pub const VALUE_POOL_FLOW_HISTORY_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("value_pool_flow_history");
/// Initial consumer-local schema.
pub const VALUE_POOL_FLOW_HISTORY_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl ValuePoolFlowHistoryConsumer {
    ...
    ) -> Result<Option<ValuePoolFlowBackfillCoverage>, DeriveStoreError> {
        store
    ...
    pub fn tail_coverage(
        store: &crate::DeriveStore,
    ) -> Result<Option<ValuePoolFlowTailCoverage>, DeriveStoreError> {
        store
            .get_consumer(
                VALUE_POOL_FLOW_HISTORY_COVERAGE_COLUMN_FAMILY,
                TAIL_COVERAGE_KEY,
...
fn validate_tail_seed_batch(
    ...
    let tail = ValuePoolFlowHistoryConsumer::tail_coverage(store)?.ok_or_else(|| {
        Box::new(ValuePoolFlowHistoryConsumerError::CoverageDiscontinuous) as DeriveConsumerError
    })?;
    let expected = tail
        .complete_through_height
...
fn store_decode_error(error: &ValuePoolFlowHistoryConsumerError) -> DeriveStoreError {
...
mod tests {
    ...
    #[test]
    fn signed_balances_round_trip_and_derive_neutral_classification()
    -> Result<(), ValuePoolFlowHistoryConsumerError> {
