...
pub const COMMITMENT_ROOT_SEARCH_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("commitment_root_search");

/// Initial on-disk schema for final-root lookup and coverage rows.
pub const COMMITMENT_ROOT_SEARCH_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl CommitmentRootSearchConsumer {
    ...
    pub fn search(
        ...
        max_matches: usize,
    ) -> Result<Vec<CommitmentRootIndexEntry>, DeriveStoreError> {
        if max_matches == 0 {
            return Ok(Vec::new());
        }
    ...
    pub fn backfill_coverage(
        store: &DeriveStore,
    ) -> Result<Option<CommitmentRootBackfillCoverage>, DeriveStoreError> {
        let Some(payload) =
    ...
    /// derive backfill have their own resumable progress contract.
...
fn decode_index_entry(
    key: &[u8],
    payload: &[u8],
) -> Result<CommitmentRootIndexEntry, DeriveStoreError> {
...
fn decode_coverage(payload: &[u8]) -> Result<CommitmentRootBackfillCoverage, DeriveStoreError> {
