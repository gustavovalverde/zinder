...
pub const VALUE_POOL_BALANCE_HISTORY_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("value_pool_balance_history");
/// Initial consumer-local schema.
pub const VALUE_POOL_BALANCE_HISTORY_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl ValuePoolBalanceHistoryConsumer {
    ...
    pub fn backfill_coverage(
        store: &DeriveStore,
    ) -> Result<Option<ValuePoolBalanceBackfillCoverage>, DeriveStoreError> {
        store
            .get_consumer(
                VALUE_POOL_BALANCE_HISTORY_METADATA_COLUMN_FAMILY,
    ...
    pub fn tail_coverage(
        store: &DeriveStore,
    ) -> Result<Option<ValuePoolBalanceTailCoverage>, DeriveStoreError> {
        store
            .get_consumer(
                VALUE_POOL_BALANCE_HISTORY_METADATA_COLUMN_FAMILY,
                TAIL_COVERAGE_KEY,
...
fn validate_tail_seed_batch(
    ...
    let tail = ValuePoolBalanceHistoryConsumer::tail_coverage(store)?.ok_or_else(|| {
        Box::new(ValuePoolBalanceHistoryConsumerError::CoverageDiscontinuous) as DeriveConsumerError
    })?;
    let expected = tail
        .complete_through_height
...
fn store_decode_error(error: &ValuePoolBalanceHistoryConsumerError) -> DeriveStoreError {
