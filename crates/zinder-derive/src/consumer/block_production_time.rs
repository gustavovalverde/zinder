...
use crate::{ConsumerProjectionState, DeriveStore, DeriveStoreError, DeriveStoreReadSnapshot};
...
pub const BLOCK_PRODUCTION_TIME_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("block_production_time");

/// Initial persisted schema for block production times.
pub const BLOCK_PRODUCTION_TIME_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl BlockProductionTimeConsumer {
    ...
    pub fn backfill_coverage(
        store: &DeriveStore,
    ) -> Result<Option<BlockProductionTimeBackfillCoverage>, DeriveStoreError> {
    ...
    pub fn tail_coverage(
        store: &DeriveStore,
    ) -> Result<Option<BlockProductionTimeTailCoverage>, DeriveStoreError> {
    ...
    /// Reads one indexed row by height from a stable derive-store snapshot.
...
fn validate_tail_seed_batch(
    ...
    let tail = BlockProductionTimeConsumer::tail_coverage(store)?.ok_or_else(|| {
        Box::new(BlockProductionTimeConsumerError::CoverageDiscontinuous) as DeriveConsumerError
    })?;
    let expected = tail
        .complete_through_height
        .map_or(Some(tail.boundary_height), BlockHeight::next);
...
fn coverage_decode_error(error: &BlockProductionTimeConsumerError) -> DeriveStoreError {
