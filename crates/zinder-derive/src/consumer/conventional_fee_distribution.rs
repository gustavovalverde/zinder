...
pub const CONVENTIONAL_FEE_DISTRIBUTION_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("conventional_fee_distribution");

/// Initial consumer-local schema. Existing derive consumers are unaffected.
pub const CONVENTIONAL_FEE_DISTRIBUTION_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl ConventionalFeeDistributionConsumer {
    ...
    pub fn backfill_coverage(
        store: &DeriveStore,
    ) -> Result<Option<ConventionalFeeDistributionBackfillCoverage>, DeriveStoreError> {
    ...
    pub fn tail_coverage(
        store: &DeriveStore,
    ) -> Result<Option<ConventionalFeeDistributionTailCoverage>, DeriveStoreError> {
        let Some(payload) = store.get_consumer(
            CONVENTIONAL_FEE_DISTRIBUTION_COVERAGE_COLUMN_FAMILY,
            TAIL_COVERAGE_KEY,
        )?
...
fn store_decode_error(error: &ConventionalFeeDistributionConsumerError) -> DeriveStoreError {
