//! `IronwoodMigration` derive consumer.
//!
...
//! derive store. On a from-genesis rebuild that baseline is the genesis block,
//! so the totals are chain-absolute; on a derive store first populated against
...
pub const IRONWOOD_MIGRATION_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("ironwood_migration");

/// On-disk schema declaration for the Ironwood-migration derive consumer.
pub const IRONWOOD_MIGRATION_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl IronwoodMigrationConsumer {
    ...
    pub fn read_pool_totals_at_or_before(
        store: &DeriveStore,
        height: BlockHeight,
    ) -> Result<Option<MigrationPoolTotals>, DeriveStoreError> {
    ...
    pub fn read_migrations_in_range(
        ...
        limit: usize,
    ) -> Result<Vec<Migration>, DeriveStoreError> {
    ...
    fn seed_running_total_for_apply(
        ...
        store: &DeriveStore,
        height: BlockHeight,
    ) -> Result<PoolBalances, DeriveStoreError> {
...
) -> Result<PoolBalances, DeriveConsumerError> {
...
fn decode_pool_totals_value(
    height: u32,
    payload: &[u8],
) -> Result<MigrationPoolTotals, DeriveStoreError> {
...
fn decode_migration(key: &[u8], payload: &[u8]) -> Result<Migration, DeriveStoreError> {
...
fn decode_height_key(key: &[u8]) -> Result<u32, DeriveStoreError> {
...
fn malformed_row(column_family: &str, actual: usize, expected: usize) -> DeriveStoreError {
...
///
/// Infrastructure failures (store I/O, derive-store writes) reach the SDK
/// through the boxed [`DeriveConsumerError`] without going through this enum.
