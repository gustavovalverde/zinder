...
pub const TRANSACTION_COMPONENT_SUMMARY_CONSUMER_NAME: DeriveConsumerName =
    DeriveConsumerName::from_static("transaction_component_summary");
...
/// derive store clears and rebuilds this consumer only.
pub const TRANSACTION_COMPONENT_SUMMARY_SCHEMA: DeriveConsumerSchema = DeriveConsumerSchema::new(
...
impl TransactionComponentSummaryConsumer {
    ...
    pub fn tail_coverage(
        store: &DeriveStore,
    ) -> Result<Option<TransactionComponentTailCoverage>, DeriveStoreError> {
        let Some(payload) = store.get_consumer(
            TRANSACTION_COMPONENT_SUMMARY_COVERAGE_COLUMN_FAMILY,
            TAIL_COVERAGE_KEY,
        )?
    ...
    /// newly added consumer inherits from existing derive consumers. Reorg
...
fn store_decode_error(error: &TransactionComponentSummaryConsumerError) -> DeriveStoreError {
...
pub enum TransactionComponentSummaryConsumerError {
    ...
    /// Derive-store read or write failure.
    #[error(transparent)]
