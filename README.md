...
- **Explorer plane** (`zinder-explorer`, optional). Serves block summaries, transaction details, typed search, mempool dashboards, and other explorer-shaped reads through `ExplorerQuery`. Before binding traffic it admits one native Wallet endpoint and one explorer materialized-view secondary with the same canonical construction identity; an optional Node endpoint supplies freshness observation only. `zinder-materialized-views` owns the reusable store and consumers, and `zinder-ingest` is their sole production writer. See [explorer plane](docs/architecture/explorer-plane.md) and [materialized-view plane](docs/architecture/materialized-view-plane.md).
