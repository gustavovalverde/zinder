...
The data-plane shape does not require abandoning lightwalletd. Zinder ships a discrete `CompactTxStreamer` adapter speaking the vendored v0.5.0 protocol, covered by integration and live-parity suites against the reference lightwalletd. The native typed plane is the product; the lightwalletd wire is a translation layer over the same stored data. Backward compatibility is a coverage contract the plane satisfies, not a constraint the plane is shaped by.

That distinction sets the migration cost for the clients that exist today. The installed light clients, Zingolib and Zodl through its SDK, reach their indexer over the `CompactTxStreamer` wire. Moving them is an endpoint change with parity validation, not a rewrite.

The deepest integration is Zallet, which embeds Zaino as an in-process library rather than calling it over a wire. A compatibility shim cannot swap an in-process dependency, so that case deserves an honest accounting. At the call surface the coupling is shallow: Zallet uses roughly nine distinct indexer methods, and the typed client now covers all of them. The last gap was a verbose mined-transaction read that returns serialized bytes. `WalletQuery.Transaction` now carries those bytes on its mined arm alongside the mined block hash, height, block time, and epoch-bound confirmations, so the verbose read is one typed call against stored data rather than a separate RPC. The most useful fact is that Zallet's own stated direction is to consume an abstract chain-index trait over a chain snapshot, which is structurally what the typed client already is. The migration aligns with where Zallet says it wants to go.
