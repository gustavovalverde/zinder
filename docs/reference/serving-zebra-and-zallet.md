...
3. **Mempool index contract.** Insert-only is forbidden by RFC-0001, but the queryable-index requirements (presence, mempool spends, mempool UTXOs by address) are now grounded in Zallet evidence. [ADR-0010](../adrs/0010-mempool-topology-and-retention.md) picks `MempoolSnapshot` plus `MempoolEvents`; future work should refine payload fields and retention behavior, not reopen the basic surface split without new evidence.
