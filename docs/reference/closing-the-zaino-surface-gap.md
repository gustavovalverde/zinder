...
**Closed by** M5 Slice B + [ADR-0011: Derive-plane federation pattern](../adrs/0011-derive-plane-federation-pattern.md). The federated `WalletQuery.TransparentAddressBalance` RPC, `ExplorerQuery.TransparentAddressBalance` direct surface, capability `derive.explorer.transparent_balance_v1`, `zinder-core::TransparentAddressBalance` domain type, `ChainIndex::transparent_address_balance` on `LocalChainIndex` and `RemoteChainIndex`, and the compat shim wiring of `GetTaddressBalance` + per-address-loop `GetTaddressBalanceStream` all shipped on 2026-05-09. The compute path is Shape C (compute at read time, federated UTXO sum + M3 mempool overlay) per the [M5 spec §D4 implementation note](../specs/m5-transparent-address-balance.md); the accumulator-backed Shape A is reserved as a future read-path optimization that does not change the public wire shape.
