...
Per-consumer evidence was cross-referenced against [Android wallet integration findings](android-wallet-integration-findings.md) for Zashi/Zodl, [Serving public lightwalletd clients](serving-public-lightwalletd-clients.md) for `zec.rocks`-style operators, and [ADR-0013](../adrs/0013-derive-plane-instantiation-and-transparent-address-balance.md) for the derive-plane shape that closed G1. Citations point at file:line in the local trees. Issue numbers are linked at first use.
...
### G1. Transparent-address balance is `Status::unimplemented` ✓ closed by Primitive D

**Closed by** M5 Slice B + [ADR-0011: Derive-plane federation pattern](../adrs/0011-derive-plane-federation-pattern.md) + [ADR-0013: Derive-plane instantiation and transparent address balance](../adrs/0013-derive-plane-instantiation-and-transparent-address-balance.md). The federated `WalletQuery.TransparentAddressBalance` RPC, `ExplorerQuery.TransparentAddressBalance` direct surface, capability `derive.explorer.transparent_balance_v1`, `zinder-core::TransparentAddressBalance` domain type, `ChainIndex::transparent_address_balance` on `LocalChainIndex` and `RemoteChainIndex`, and the compat shim wiring of `GetTaddressBalance` + per-address-loop `GetTaddressBalanceStream` all shipped on 2026-05-09. The compute path is Shape C (compute at read time, federated UTXO sum + M3 mempool overlay) per [ADR-0014](../adrs/0014-compute-at-read-time-canonical-reads.md); the accumulator-backed Shape A is reserved as a future read-path optimization that does not change the public wire shape.

**Historical evidence (kept as anchor).** Before M5 Slice B landed, the lightwalletd-compat adapter rejected `GetTaddressBalance` and `GetTaddressBalanceStream` with `Status::unimplemented` at `services/zinder-compat-lightwalletd/src/grpc.rs`. Native `WalletQuery` had no `TransparentAddressBalance` RPC. `ChainIndex` had no `transparent_address_balance` method. The Zaino reference path (`ReadStateService::AddressBalance` against finalized state only) carried no mempool view and dropped the `received` total at the lightwalletd compat boundary because `lightwalletd::Balance` carries only `value_zat`.

**Affected consumers (resolved).**

- Zashi/Zodl: receives the structured `confirmed_zat` + signed `unconfirmed_delta_zat` split via the federated native method, closing the Esplora-shaped balance UI gap per [Android wallet integration findings](android-wallet-integration-findings.md).
- Public lightwalletd operators: `GetTaddressBalance` answers from the federated path through `balance_response_from_proxy`; deployments without `zinder-derive` reachable get gRPC `UNAVAILABLE` and the capability is omitted from `ServerInfo` rather than silently failing.
- Block explorers: call `ExplorerQuery.TransparentAddressBalance` directly, with the per-request 256-address cap as the bound on compute fanout.
- Zallet remains unaffected; Zallet computes balance from the wallet DB and never called the gap'd RPC.

**Pattern.** [Pattern 6: Performance as a Sequential Implementation](lessons-from-zaino.md). Shape C reuses M4's transparent UTXO surface and M3's mempool point lookups instead of materializing a fresh column family; Shape A is the future optimization the wire shape and capability already accommodate without change.
...
### G6. Transparent-mempool surfaces are Rust-trait-only on the gRPC boundary ✓ closed by Primitive C
...
### G18. No prevout-resolution surface ✓ closed by M6 + [ADR-0014](../adrs/0014-compute-at-read-time-canonical-reads.md)

**Closed by** the paired `WalletQuery.TransparentPrevouts` (canonical) and `WalletQuery.TransparentMempoolPrevouts` (live mempool) RPCs shipped across M6 Slices 1-3 and the [ADR-0014: Compute-at-read-time read-path pattern](../adrs/0014-compute-at-read-time-canonical-reads.md) that locks the durable read-path contract. The canonical handler reads existing `TransactionArtifact` payload bytes via Shape C compute-at-read-time (zero new column families), parses each transaction once via `zinder_source::transparent_prevout_from_raw_transaction_bytes`, and returns one `TransparentPrevoutEntry` per requested outpoint in input order. The mempool handler reads `MempoolEntry.transparent_outputs` directly through `MempoolIndex::transparent_prevouts_by_outpoints` (no parsing at read time). The shared `OutPoint` proto message normalizes the wire-level outpoint shape across `TransparentMempoolSpendByOutpoint` (M3), `TransparentPrevouts` (M6 canonical), and `TransparentMempoolPrevouts` (M6 mempool). Capabilities `wallet.read.transparent_prevouts_v1` and `wallet.mempool.transparent_prevouts_v1`. Coinbase-sentinel inputs are rejected with `INVALID_ARGUMENT` per [Anti-Pattern A4](#a4-sentinel-overloaded-blockid--height-0-hash-bytes-). Per-request cap is `MAX_TRANSPARENT_PREVOUTS_PER_REQUEST = 256`, mirroring the M5 balance address cap.

**Historical evidence (kept as anchor).** `wallet/zallet/src/components/json_rpc/methods/view_transaction.rs:443-477`. To compute the input value of a transparent input, Zallet called `chain.get_raw_transaction(prev_txid, Some(1))` once per input, fetching the entire previous transaction just to extract one `TxOut` from its `vout` array. For an N-input transaction this was N round trips with no batching.

Neither Zaino nor pre-M6 Zinder exposed a "given this `OutPoint`, return the `TxOut`" call. There was no single-`OutPoint` lookup and no batched prevout-resolution surface.
...
4. **✓ shipped: native gRPC mirroring of `ChainIndex` mempool point lookups (G6).** Promoted focused mempool point lookups (`WalletQuery.TransparentMempoolOutputsByAddress` capability `wallet.mempool.transparent_outputs_by_address_v1`, `WalletQuery.TransparentMempoolSpendByOutpoint` capability `wallet.mempool.transparent_spend_by_outpoint_v1`) to native gRPC. The capability namespace is `wallet.mempool.*` per the rule that storage tier and lifecycle differ from canonical address-keyed reads. The handlers proxy through `IngestControl` per [ADR-0010](../adrs/0010-mempool-topology-and-retention.md) so the writer-owned mempool index is the single source of truth, and the same primitives are now what Shape C of M5 Slice B uses to compose the unconfirmed delta.
