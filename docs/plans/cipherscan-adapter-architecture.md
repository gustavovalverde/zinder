...
- Viewing-key scanning, participant identity claims, adversarial privacy
  guarantees, or safety verdicts. The adapter may own deterministic Cipherscan
  linkability scoring over complete native flow facts because that policy is
  part of the product contract, not a canonical chain fact.
...
| `/api/tx/:txid/linkability` | Classifies the source flow from signed transaction-intrinsic balances, reads the bounded rolling flow window, applies Cipherscan's v2 candidate and scoring policy, and hydrates standard transparent addresses at the same chain epoch. A result is returned only when native coverage is complete, neither scan nor result bounds truncated the range, and the source event is present. Incomplete analysis returns `503` with the known source facts and no score or no-match verdict. | `ExplorerQuery.TransactionDetail`, `ExplorerQuery.ValuePoolFlowEventsInRange` |
...
| Privacy graph, cluster, batch-risk, recommendation, peer/node inventory, and chain-size endpoints | Return stable degraded JSON with `degraded` and `unavailable` markers. | Graph and identity analytics, node inventory, recommendations, and physical chain-size history are sidecar-owned or unavailable. |
...
| `/api/tx/:txid/linkability` | Implements Cipherscan's v2 transaction-linkability contract over complete, bounded, epoch-coherent native flow facts. For shielding transaction `fe18003a...34852f2`, local and public testnet return the same source classification, 1,124,945,000-zatoshi amount, Sapling pool, transparent address, empty candidate list, `LOW` warning, score 0, and algorithm metadata. Both unchanged transaction pages render `Clear · 0/100` and `no round-trip` without console errors. | Complete without a persistent schema change. `ExplorerQuery.ValuePoolFlowEventsInRange` adds a reusable read over the existing flow projection with half-open time bounds, typed direction and pool filters, inclusive amount bounds, explicit scan/result truncation, coverage, freshness, and chain-epoch stabilization. The adapter owns Cipherscan's candidate policy, scores, warnings, JSON, and truthful unavailable response. This analysis does not prove identity or provide a safety guarantee. |
| `/api/privacy/linkage-edges`, `/api/privacy/batch-risks`, `/api/privacy/clusters`, `/api/privacy/graph/:txid`, `/api/privacy/shield/:txid/batch`, `/api/privacy/patterns`, `/api/privacy/recommended-swap-amounts` | Matches validation errors and returns stable empty degraded graph, linkage, cluster, batch-risk, and recommendation envelopes. | Real graph, cluster, identity-risk, and swap-recommendation results require Cipherscan-owned sidecars and remain outside Zinder core. |
...
| Transaction page | `/api/tx/:txid` | Completed adapter surface plus completed `TransactionDetail` v3 contract and paid-fee correction | `ExplorerQuery.TransactionDetail` | Mined canonical rows provide transparent outputs with canonical spent state and ordered inputs with outpoint, independently optional value, and script. The adapter decodes standard input/output addresses, retained coinbase miner data, aggregate values, Cipherscan count aliases, and signed Sapling, Orchard, and Ironwood balances. Safe fee reads preserve compatible historical rows and recover missing values from the same parent-fact batch without a projection replay. |
...
| Transaction linkability | `/api/tx/:txid/linkability` | Completed adapter scoring over bounded, complete, epoch-coherent native flow facts; returns truthful unavailability when those preconditions are not met | `TransactionDetail`, `ValuePoolFlowEventsInRange` | Keep canonical source facts, bounds, freshness, and coverage in Zinder. Keep Cipherscan's candidate policy, scores, warnings, and JSON in the adapter. Do not present the result as identity proof or a safety guarantee. |
| Privacy graph and risk products | `/api/privacy/*` graph, cluster, batch-risk, pattern, and recommendation endpoints | Degraded adapter responses; sidecar required for real product analytics | none | Graph relationships, identity-oriented risk products, and recommendations remain Cipherscan-owned. |
...
| Sidecar-owned | Turnstile, labels, historical prices beyond configured provider coverage, names, cross-chain data, mining attribution, Crosslink/finalizer data, identity-oriented risk products, and graph analytics | Cipherscan or external product sidecar; no Zinder schema. Bounded Cipherscan-compatible transaction-linkability scoring remains adapter-owned. |
