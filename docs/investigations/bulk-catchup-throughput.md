...
curl -fsS http://127.0.0.1:9105/metrics \
  | grep -E 'zinder_ingest_(derive_duration_seconds|batch_accumulator_(blocks|transactions|transparent_outputs|transparent_spend_references)|backfill_stage_duration_seconds|commit_(stage_duration_seconds|batch_(block|transaction|transparent_output|transparent_spend_reference)_count)|derive_replay_stage_duration_seconds|derive_tailer_tick_duration_seconds|transparent_spend_fact_|raw_blob_disabled_total)'
```
