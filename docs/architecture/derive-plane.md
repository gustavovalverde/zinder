...
The rebuild path is always available; it is the failsafe. Scoping rebuild to the one consumer whose version moved keeps an unrelated consumer's data intact, which matters once canonical retention is clamped to derive progress: a whole-store rebuild would otherwise force a from-genesis canonical re-ingest.
