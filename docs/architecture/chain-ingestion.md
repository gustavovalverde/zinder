...
6. **Bounded derive replay.** Startup derive replay uses `ingest.derive.replay_batch_blocks`, `ingest.derive.replay_policy`, memory watermarks under `[ingest.derive]`, and an internal variable-projection row cap. Canonical-first replay shrinks the effective batch size at `memory_degrade_ratio`, pauses at `memory_pause_ratio`, resumes from pause as degraded work below `memory_pause_ratio`, and returns to the normal batch size below `memory_resume_ratio`; `min_replay_batch_blocks` is the lowest degraded batch size before pause. When memory state is normal and the current replay chunk has low projected fan-out, ingest hydrates one following replay batch ahead of dispatch while preserving serial derive cursor advancement.
   ...
   A canonical-phase gate throttles replay for **both** policies while the unified loop is in `BulkCatchup` (the canonical gap exceeds `ingest.phases.catchup_threshold_blocks`). The derive tailer reads the loop's current phase from the shared readiness handle and, while the gate is engaged, drops the effective batch to `min_replay_batch_blocks` so canonical bulk catch-up owns the storage and memory budget. Replay is throttled to residual capacity, never fully paused by the gate: the tailer keeps publishing the canonical retention release floor and persisting `DeriveStatus`, so the safe-tip sweep keeps deleting settled spend facts (a frozen floor would stall the sweep and let spend-fact rows grow unbounded through a multi-hour rebuild) and the operator-facing head stays truthful. Memory pressure composes with the gate, and the stricter of the two wins: pressure at `memory_pause_ratio` still pauses replay entirely below the gate's residual floor. `DeriveReplayPolicy::Continuous` keeps its meaning only as an at-tip override; it no longer bypasses throttling during a from-genesis rebuild where canonical bulk catch-up is itself the dominant memory consumer. Derive replay logs one WARN when the gate first engages under `continuous`, sampled on a fixed cadence independent of how long one replay pass runs, and the gate's engaged state is exported on `zinder_ingest_derive_replay_phase_gate`.
7. **Bulk-catchup memory headroom.** Canonical bulk-catchup batches used to run back-to-back with no yield point between them. After each committed batch, the unified loop now samples the same runtime memory-pressure ratio the derive-replay budget uses and, at or above `memory_pause_ratio`, holds until pressure drops back below `memory_resume_ratio`; between `memory_degrade_ratio` and `memory_pause_ratio` it applies one short backoff. This reuses `[ingest.derive]`'s ratios so operators tune one set of thresholds for both consumers of the shared memory budget, and it applies regardless of `replay_policy`.
