...
- `MempoolIndex` / `MempoolEventLog`: non-canonical mempool view and event stream, implemented outside `commit_ingest_batch`. The live index is in-memory; the event log persists through the `mempool_event` column family per [ADR-0007](../adrs/0007-mempool-topology-and-retention.md). Both are owned by `zinder-ingest` and spawn once at process start, alongside the private `IngestControl`/`CanonicalControl` servers, when the ingest binary's `--ingest-control-listen-addr` flag is set. `LiveMempoolOwner` (`run_live_mempool_owner`) is a sibling task of the unified ingest loop: it consumes a `MempoolSource` stream, hydrates each observation through `build_mempool_entry`, and durably appends typed `Added`/`Invalidated`/`Mined` envelopes to `MempoolEventLog` through the canonical writer's control channel, so the process-local index never opens a second primary. It stays hydrating and fails closed until its source's initial-snapshot marker lets it reconcile against the durable log and start serving. A separate retention task (`run_mempool_retention`) prunes per-variant windows on a fixed interval through the same control channel and emits `MempoolCursorAtRisk` readiness when the oldest retained sequence approaches the configured floor; this is the mempool-side equivalent of [chain-event retention](chain-events.md#retention-and-backpressure).
...
Transitions are bidirectional. A long downtime or upstream burst that re-opens the gap beyond the threshold returns the loop to `BulkCatchup` after the current serial iteration, then stays there until the gap closes. The live mempool owner and its retention task spawn once at process start, independent of phase transitions; `TipFollow` polls the upstream tip on a fixed interval rather than subscribing to a chain-tip notification stream, waking early only on mempool hydration changes. Transparent-projection retention is separately gated behind canonical and derive completion. The `IngestControl` gRPC server starts at process start and runs throughout.
