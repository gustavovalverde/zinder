...
| `WalletQueryApi` | Rust query boundary implemented behind compatibility adapters |
...
| `ServerCapabilities` | Capability descriptor advertised by the serving adapter to clients |
...
- `zinder-serve` as a crate or deployable boundary. Use `zinder-ingest`, `zinder-projector`, or `zinder-compat-lightwalletd`.
...
[ops]
# Per-service default: 127.0.0.1:9105 (ingest), :9110 (projector),
# :9107 (compat). Explorer remains post-cutover work on :9069.
...
[storage]
path = "/var/lib/zinder"
# Reader-only canonical secondary knobs (zinder-compat-lightwalletd):
secondary_path = "/var/lib/zinder/compat/canonical-secondary"
...
# Reader-side (zinder-projector, zinder-compat-lightwalletd): writer URL.
...
[retention]
# Enforced by `zinder-ingest`; one section, one source of truth.
...
# Closed derive workload. Defaults to "explorer". Selecting a different preset
# requires a fresh canonical-plus-projection store.
projection_preset = "explorer" # "wallet" | "explorer"
...
`[retention]` is the single source of truth enforced by `zinder-ingest`.
`[ingest_control]` is shared: the writer reads `listen_addr` to bind, the
projector and compatibility reader use `addr` to dial, and all three use
`bearer_token_path` when ADR-0006 auth is enabled.

The compatibility reader must set `compat.reorg_window_blocks` to the writer's `ingest.reorg_window_blocks`. The value is part of the immutable canonical-store identity, so `zinder-compat-lightwalletd` refuses startup when the configured expectation differs from the persisted writer policy. It cannot infer or silently adopt the persisted value because that would remove the operator's admission check.
...
| `ZINDER_STORAGE__DERIVE__ROCKSDB__BLOCK_CACHE_BYTES` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.block_cache_bytes` | Derive-store RocksDB block cache budget in bytes. Defaults to 268435456 for writers and 67108864 for readers. |
| `ZINDER_STORAGE__DERIVE__ROCKSDB__MAX_WAL_BYTES` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.max_wal_bytes` | Derive-store RocksDB live WAL ceiling in bytes. Defaults to 268435456 for writers and 16777216 for readers. |
| `ZINDER_STORAGE__DERIVE__ROCKSDB__MAX_OPEN_FILES` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.max_open_files` | Derive-store RocksDB open SST file cap. Defaults to 512 for writers and 64 for readers. |
| `ZINDER_STORAGE__DERIVE__ROCKSDB__WRITE_BUFFER_BYTES` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.write_buffer_bytes` | Derive-store per-column-family RocksDB write buffer size. Defaults to 16777216 for writers and 4194304 for readers. |
| `ZINDER_STORAGE__DERIVE__ROCKSDB__MAX_WRITE_BUFFER_COUNT` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.max_write_buffer_count` | Derive-store per-column-family mutable plus immutable RocksDB write buffer count. Defaults to 4 for writers and 2 for readers. |
| `ZINDER_STORAGE__DERIVE__ROCKSDB__MAX_BACKGROUND_JOBS` | zinder-ingest, zinder-projector, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.max_background_jobs` | Derive-store primary-writer RocksDB background job cap shared by flush and compaction work. Defaults to 2 and is not applied to secondary opens. |
...
| `ZINDER_INGEST__DERIVE__STARTUP_HANDOFF_LAG_BLOCKS` | zinder-ingest | Optional | `ingest.derive.startup_handoff_lag_blocks` | Residual derive lag in blocks at which a FollowingTip startup stops replaying synchronously and hands the remainder to the always-on tailer. BulkCatchup, AwaitingUpstream, failed observation, and unclassified startup states skip synchronous replay. A bounded wall-clock budget caps an admitted startup catch-up regardless of this value. Defaults to 1000. |
...
adapter. An adapter without an authenticated source probe returns an empty
node-capability list rather than guessing.
...
- Projection workload is selected independently with `ingest.projection_preset = "wallet" | "explorer"` or `zinder-ingest --projection-preset`. `explorer` is the default. The first release permits selection only for a fresh canonical-plus-projection store; a persisted mismatch fails before projection-manifest mutation and directs the operator to rebuild at a new empty storage path. Earlier preset names are rejected rather than interpreted as aliases.
