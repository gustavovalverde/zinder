...
| `ZINDER_STORAGE__PATH` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Required for ingest, query, and compatibility; optional for explorer | `storage.path` | Canonical RocksDB store path. Writers open it as primary; readers open it as a secondary. Explorer requires this field together with `storage.secondary_path` when local canonical and materialized-view reads are enabled; omit both for stateless transaction detail. |
| `ZINDER_STORAGE__SECONDARY_PATH` | zinder-ingest (verify-canonical-replay only), zinder-query, zinder-compat-lightwalletd, zinder-explorer | Required for query and compatibility; paired optional field for explorer | `storage.secondary_path` | Process-unique RocksDB secondary metadata directory. Explorer requires it together with `storage.path` when local storage is enabled. Never share this path across reader processes. |
...
| `ZINDER_EXPLORER__WALLET_QUERY_ENDPOINT` | zinder-explorer | Required for transaction detail v4; otherwise optional | `explorer.wallet_query_endpoint` | WalletQuery gRPC endpoint backing the explorer's wallet-composed reads. Explorer admits transaction detail v4 only when startup verifies the upstream service identity, network, contract revision, retained transaction bytes, transparent prevout lookup, and transparent spend lookup. Empty or unset disables wallet-composed capabilities. |
