...
| `ZINDER_NODE__AUTH__METHOD` | zinder-ingest, zinder-projector, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `node.auth.method` | Upstream-node auth shape: `none`, `basic`, or `cookie`. An omitted method is also resolved as no auth. |
...
| `ZINDER_NODE__HEALTH__ADDR` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `node.health.addr` | URL of the upstream's HTTP `/ready` endpoint. When set, ingest and the wallet-serving readers poll it as the primary upstream-sync signal; Explorer records it only as an upstream freshness observation. When unset, they fall back to `getblockchaininfo.verificationprogress`/`estimatedheight`. See [ADR-0015](../adrs/0015-phase-driven-ingest.md). |
...
| `ZINDER_NODE__HEALTH__VERIFICATION_PROGRESS_FLOOR` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `node.health.verification_progress_floor` | Lower bound on `getblockchaininfo.verificationprogress` below which the fallback path reports the node as not ready. Explorer records that outcome only as freshness. Defaults to 0.999. Must be in `(0.0, 1.0)`; Explorer requires `node.health.addr` when this setting is present. |
| `ZINDER_NODE__HEALTH__ESTIMATED_GAP_FLOOR_BLOCKS` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `node.health.estimated_gap_floor_blocks` | Block gap between `estimatedheight` and the local tip above which the fallback path reports the node as not ready. Explorer records that outcome only as freshness. Defaults to 10; Explorer requires `node.health.addr` when this setting is present. |
...
| `ZINDER_STORAGE__MATERIALIZED_VIEWS__ROCKSDB__MAX_BACKGROUND_JOBS` | zinder-ingest | Optional | `storage.materialized_views.rocksdb.max_background_jobs` | Materialized-view primary-writer background flush and compaction job cap. Defaults to 2. |
...
persisted consumer identities, and concrete providers. Temporary dependency health and
