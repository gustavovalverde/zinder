...
| `ZINDER_STORAGE__CANONICAL__ROCKSDB__MAX_BACKGROUND_JOBS` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.canonical.rocksdb.max_background_jobs` | Canonical-store aggregate RocksDB background job cap shared by flush and compaction work. Defaults to 2. |
...
| `ZINDER_STORAGE__DERIVE__ROCKSDB__MAX_BACKGROUND_JOBS` | zinder-ingest, zinder-query, zinder-compat-lightwalletd, zinder-explorer | Optional | `storage.derive.rocksdb.max_background_jobs` | Derive-store aggregate RocksDB background job cap shared by flush and compaction work. Defaults to 2. |
