...
| Mined transaction detail and bytes: `/api/tx/:txid`, `/api/tx/:txid/raw`, `/api/tx/raw/batch`, `/api/tx/:txid/verbose` | Unavailable in the operator-built Explorer binary composition | `ExplorerQuery.TransactionDetail`, `WalletQuery.Transaction`, transaction-byte retention | Map inputs, outputs, spent state, fee fields, pool balances, and raw-byte envelopes; require the explorer capability before serving dependent routes | The release wallet endpoint must first own and advertise native transaction lookup plus transaction bytes; retained blobs alone are insufficient | Zinder, adapter, unavailable |
...
| Mempool and realtime: `/api/mempool`, `/api/mempool/tx/:txid`, root WebSocket | Partial; transaction-detail-dependent paths are unavailable in the operator-built Explorer binary composition | `MempoolSnapshot`, `IngestControl.MempoolTransaction`, `ExplorerQuery.TransactionDetail`, `WalletQuery.ChainEvents`, `WalletQuery.MempoolEvents` | Preserve mempool fallback behavior and `new_block`, `mempool_tx`, and `mempool_removed` frames; manage reconnect cursors internally; gate detail-dependent paths on discovery | Native transaction lookup and bytes must be admitted before detail-dependent responses can be certified; parent-input enrichment, shielded balances, and paid fees for pending transactions remain unavailable | Zinder, adapter, unavailable |
...
| Blend, common amounts, and transaction linkability: `/api/blend-check`, `/api/blend-check/split`, `/api/privacy/common-amounts`, `/api/tx/:txid/linkability` | Partial; transaction-linkability paths are unavailable while TransactionDetail is omitted | Value-pool amount-threshold and rounded-amount summaries, `TransactionDetail`, `ValuePoolFlowEventsInRange` | Candidate policy, scoring, split rules, labels, coercion, cache, warnings, and JSON; gate detail-dependent routes on discovery | Results are product heuristics, not identity or safety claims; native transaction lookup and bytes are required for transaction-linkability certification | Zinder, adapter, unavailable |
...
With the operator-built Explorer binary composition, an Explorer TransactionDetail call
returns gRPC `UNIMPLEMENTED`. The current adapter translates that result to
HTTP `502` with `code = "upstream_error"` for `/api/tx/:txid`,
`/api/mempool/tx/:txid`, and `/api/tx/:txid/linkability` when its earlier
capability gates pass. The release WalletQuery applies the same native
transaction guard to `/api/tx/:txid/raw` and
`/api/tx/:txid/verbose`, so those routes also translate the result to the same
`502` shape. `/api/tx/raw/batch` catches each guarded lookup and returns its
normal HTTP `200` batch envelope with each affected transaction in `failed`.
These translations describe the current gap; they are not the intended
consumer contract. The compatibility runtime must either admit a real wallet
transaction-plus-bytes owner or fail its own startup admission before binding.
