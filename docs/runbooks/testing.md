...
| Testnet upstream-readiness calibration | Medium | Configure Zebra with `ready_max_blocks_behind = 4`, `ready_max_tip_age = "5m"`, at least 1 live peer, and readiness enforcement on Testnet. Sample Zebra `/ready`, `getblockchaininfo`, all Zinder `/readyz` endpoints, and the trusted-TLS compatibility route through ordinary block gaps; then introduce each upstream failure condition separately. | Ordinary gaps below 5 minutes do not close the route while Zebra remains canonically current. Excessive tip age, actual chain lag, peer loss, incomplete synchronization, and an unavailable health endpoint each drain readiness, and the route recovers after the upstream condition clears. | Exact Zebra config and image identity; timestamped `/ready`, RPC, Zinder readiness, and trusted-TLS samples; consumer progress; intentional-failure start, recovery, and attribution. | [Deploying the wallet-serving container §Failure handling](deploying-wallet-serving.md#failure-handling), [Trusted TLS and ZODL compatibility admission](zodl-trusted-tls-certification.md) |
...
- [ ] Testnet upstream-readiness calibration proves that the 4-block estimator
      allowance prevents normal-gap flaps while peer loss, incomplete sync,
      actual lag, and tips older than 5 minutes still fail closed.
