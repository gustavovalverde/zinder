...
`ingest.bulk_catchup.block_prepare_concurrency` controls CPU workers; `block_prepare_memory_watermark_bytes` is the admission watermark for prepare peak estimates, ordered prevout resolution, and resident commit-preparation handoff data. Active work reserves a conservative peak estimate before parsing. Completed work retains the larger of that peak or measured resident canonical facts, replay, compact data, and retained raw blobs through prevout resolution, then resizes to the resident handoff until commit reassembly takes ownership. Recent-output cache entries carry separate reservations under the same watermark. The watermark is not a hard allocator cap: one oversized block or a measured result above its estimate is allowed to finish, records `zinder_ingest_bulk_pipeline_watermark_oversized_total`, and stops further prepare admission until the reservation falls below the limit. The source and commit-reassembly byte limits bound different memory pools and should be tuned separately. Startup derive replay is bounded by `ingest.derive.replay_batch_blocks` and the derive memory watermarks, so replay shrinks its effective batch before pausing. See [ADR-0021](../adrs/0021-parallel-block-derivation.md).
