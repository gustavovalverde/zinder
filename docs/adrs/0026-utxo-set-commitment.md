...
release serving-pair query does not implement the summary. Gate B removes the
generic query and must either remove this unowned protocol baggage or retain it
only with a concrete production consumer, authenticated admission evidence, and
an enforced resource budget. A future implementation cannot restore a manual
support flag.
