...
2. **Source errors are always loop-recoverable.** Every `SourceError` variant returns `Recover`. The only exit paths are storage failures, reorg-window violations, and internal logic errors (`EmptyIngestBatch`, etc.) where data integrity is at stake.
