...
Protobuf fits boundaries where Zinder already speaks protobuf or needs long-lived durable schema evolution. Lightwalletd compatibility requires `CompactBlock` protobuf payloads. Protobuf gives storage-control messages a field-number evolution model and cross-language repair tooling. Reserialized protobuf bytes are not canonical for semantic hashes, ordered keys, or cursor payloads.
