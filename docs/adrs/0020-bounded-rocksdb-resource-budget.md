...
`BulkCatchup` forces an explicit `db.flush()` every `flush_every_n_epochs` committed epochs (default 5). With the default `commit_batch_blocks = 1000`, this truncates the WAL after every 5,000 committed blocks. The flush is also performed once on `BulkCatchup` exit so the phase hands off a clean WAL state to `TipFollow`.
