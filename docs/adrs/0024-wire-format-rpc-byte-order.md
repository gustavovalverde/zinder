...
- **Stored proto messages flip with the wire.** Several stored artifacts in the chain store (`BlockHeaderInfo`, `TransactionLocation`, `OutPoint`, mempool/transparent-address entries) and every materialized-view payload (`BlockSummaryRecord`, `TransactionHistoryEntry`, etc.) contain the same hash-shaped fields. Their on-disk encoding changes with the proto. `CURRENT_ARTIFACT_SCHEMA_VERSION` bumps from 9 to 10 and refuses to open a store written under the older version; each affected materialized-view consumer bumps its own schema version and rebuilds its projection per [ADR-0028](0028-materialized-view-schema-versioning.md). Existing deployments must wipe and re-sync the canonical store directory before starting the new binary.
