...
`GetTaddressBalance` and `GetTaddressBalanceStream` are out of M4. They were scoped to M5 transparent-address balance and derive-plane instantiation, which is now shipped and recorded in [ADR-0013](../adrs/0013-derive-plane-instantiation-and-transparent-address-balance.md) (operational topology, consumer SDK contract, balance wire shape) and [ADR-0014](../adrs/0014-compute-at-read-time-canonical-reads.md) (compute-at-read-time pattern). The split between M4 and M5 was intentional: M4 closes the canonical wallet-shaped surfaces (UTXO and tx history); M5 closed the explorer-shaped balance surface AND instantiated the derive plane.

**Why:** Balance is an aggregation, not a per-event canonical fact. [Extending artifacts §When to add an artifact family](../architecture/extending-artifacts.md#when-to-add-an-artifact-family) names "precomputed totals table for explorer dashboards" as a derive-plane workload. Wallets compute balance client-side from `GetAddressUtxos` (Zashi today, Zallet today); the consumer that needs server-side balance is the explorer audience. Building balance canonically in M4 would set a precedent that aggregations live canonically when convenient, growing entropy across future milestones. M5 was the right home: it instantiated `services/zinder-derive` (formerly zero source files), validated the derive-plane contract end to end, and laid the foundation for future analytics workloads.

**How to apply:** M4 implementations must not anticipate M5's wire shape. Once M5 Slice B shipped, the compat shim's `GetTaddressBalance` and `GetTaddressBalanceStream` started returning real responses; the structured `TransparentAddressBalance` (with `confirmed_zat`/`unconfirmed_delta_zat`/`address_count`/`chain_epoch`) is defined in [ADR-0013 §Wire shape](../adrs/0013-derive-plane-instantiation-and-transparent-address-balance.md#wire-shape-structured-confirmed_zat-plus-signed-unconfirmed_delta_zat).
