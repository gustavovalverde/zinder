...
- [docs/architecture/service-boundaries.md](docs/architecture/service-boundaries.md): who owns what across the three deployable services (`zinder-ingest`, `zinder-projector`, `zinder-compat-lightwalletd`) plus the `zinder-query` library; `zinder-explorer` is post-cutover work, not one of the three.
